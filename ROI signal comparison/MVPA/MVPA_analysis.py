...
# -----------------------------
# Main Loop: Train/Test (store each fold separately)
# -----------------------------
output_df= pd.DataFrame(columns = ['train','test','train_type','test_type','test_subjects','accuracy'])
svc = SVC(kernel='linear')

for k, test_subjects in enumerate(test_groups):
    print(f"Fold {k+1}/{len(test_groups)}: testing {test_subjects}")

    for train_type, test_type in train_test_set:

        # --- Train ---
        if train_type == 'stimulation':
            X_train = consequence_concat_img_data2D_masked[
                [i for i, s in enumerate(consequence_event_files['subject']) if s not in test_subjects], :]
            y_train = np.array([
                consequence_event_files['sensory_labels'][i]
                for i, s in enumerate(consequence_event_files['subject'])
                if s not in test_subjects])
        elif train_type == 'decision':  # 'Decision' or 'shape'
            X_train = selection_concat_img_data2D_masked[
                [i for i, s in enumerate(selection_event_files['subject'])
                 if s not in test_subjects and selection_event_files['decision_labels'][i] != 'shape'], :]
            y_train = np.array([
                selection_event_files['sensory_labels'][i]
                for i, s in enumerate(selection_event_files['subject'])
                if s not in test_subjects and selection_event_files['decision_labels'][i] != 'shape'])
        elif train_type == 'shape':  # 'Decision' or 'shape'
            X_train = selection_concat_img_data2D_masked[
                [i for i, s in enumerate(selection_event_files['subject'])
                 if s not in test_subjects and selection_event_files['decision_labels'][i] == 'shape'], :]
            y_train = np.array([
        ...
        if test_type == 'stimulation':
            X_test = consequence_concat_img_data2D_masked[
                [i for i, s in enumerate(consequence_event_files['subject'])
            ...
            X_test = selection_concat_img_data2D_masked[
                [i for i, s in enumerate(selection_event_files['subject'])
                 if s in test_subjects and selection_event_files['decision_labels'][i] != 'shape'], :]
            y_test = np.array([
                selection_event_files['sensory_labels'][i]
                for i, s in enumerate(selection_event_files['subject'])
                if s in test_subjects and selection_event_files['decision_labels'][i] != 'shape'])
...
# -----------------------------
# Paired leave-two-subject-out folds (1 visual + 1 somatosensory per fold)
# -----------------------------
...
# Identify the two modality groups by label values present in your data
...
NFolds = len(test_groups)  # typically 40
print("NFolds (paired leave-2-out):", NFolds)
print("Example test fold:", test_groups[0])
...
for k, test_subjects in enumerate(test_groups):
    ...
    for train_type, test_type in train_test_set:
        ...
        if test_type == 'stimulation':
            X_test = consequence_concat_img_data2D_masked[
                [i for i, s in enumerate(consequence_event_files['subject']) if s in test_subjects], :]
            y_test = np.array([
                consequence_event_files['sensory_labels'][i]
                for i, s in enumerate(consequence_event_files['subject'])
                if s in test_subjects])
        ...
        output_df = pd.concat(
            [output_df,
             pd.DataFrame([[train_type, test_type, k, test_subjects, acc]],
                          columns=['train_type','test_type','fold','test_subjects','accuracy'])],
            ignore_index=True
        )

# Save
output_df.to_shapev(output_dir + ROI + '_MVPA_classification_paired_leave2out.shapev', index=False)
print("Mean accuracy across all folds:", np.mean(output_df['accuracy']))

# --- Compute and show mean ± SEM ---
summary_df = (
    output_df
    .groupby(['train_type', 'test_type'], as_index=False)['accuracy']
    .agg(['mean', 'sem'])
    .reset_index()
    .rename(columns={'mean': 'mean_accuracy', 'sem': 'sem_accuracy'})
)

print("Mean ± SEM accuracy per train/test combination:")
print(summary_df)
output_df
...
# # Permutation: label shuffling on subj-level 
...
def permute_labels_by_subject(subjects_per_sample, y_labels, rng=None):
    ...
    # Determine one (original) label per subject (assumes label constant within subject)
    subj_to_label = {}
    for subj in unique_subjects:
        lbls = y_labels[subjects_per_sample == subj]
        # use first; optionally sanity-check consistency
        subj_to_label[subj] = lbls[0]
        # Uncomment if you want to enforce consistency:
        # if not np.all(lbls == lbls[0]):
        #     raise ValueError(f"Subject {subj} has non-constant labels across samples.")

    # Permute labels across subjects
...
output_df = pd.read_shapev(output_dir + ROI + '_MVPA_classification_80folds.shapev')
n_permutations = 1000  
start_permutation = 0

perm_df = pd.DataFrame(columns=['train_type','test_type','fold','accuracy','perm','test_subjects'])
svc = LinearSVC(max_iter=5)  

if os.path.exists(output_dir + ROI + '_MVPA_classification_80folds_permutation_subjShuffle.shapev'):
    perm_df = pd.read_shapev(output_dir + ROI + '_MVPA_classification_80folds_permutation_subjShuffle.shapev')
    start_permutation = np.max(perm_df['perm'])+1
...
for perm in tqdm(range(start_permutation,n_permutations), desc="Permutation Test"):
    for k, test_subjects in enumerate(test_groups):
        ...
        for train_type, test_type in train_test_set:
            ...
            if train_type == 'stimulation':
                ...
                train_idx = [i for i, s in enumerate(selection_event_files['subject']) 
                             if s not in test_subjects and selection_event_files['decision_labels'][i] != 'shape']
    ...
    perm_df.to_shapev(output_dir + ROI + '_MVPA_classification_80folds_permutation_subjShuffle.shapev', index=False)
...
for _, pair in train_test_pairs.iterrows():
    ...
    obs_mean_acc = final_results[(final_results['train_type']==train) &
                                 (final_results['test_type']==test)]['accuracy'].mean()
    ...
    combined_results.append({
        'train_type': train,
        'test_type': test,
        'observed_mean_accuracy': obs_mean_acc,
        'p_value': p_val})
